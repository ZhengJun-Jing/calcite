 ...
 */
public class DmTable extends AbstractQueryableTable
  ...
  public final DmSchema dmSchema;
  public final String dmCatalogName;
  public final String dmSchemaName;
  public final String dmTableName;
  public final Schema.TableType dmTableType;

  DmTable(DmSchema dmSchema, String jdbcCatalogName,
      String jdbcSchemaName, String jdbcTableName,
      Schema.TableType jdbcTableType) {
    super(Object[].class);
    this.dmSchema = requireNonNull(dmSchema, "dmSchema");
    this.dmCatalogName = jdbcCatalogName;
    this.dmSchemaName = jdbcSchemaName;
    this.dmTableName = requireNonNull(jdbcTableName, "jdbcTableName");
    this.dmTableType = requireNonNull(jdbcTableType, "jdbcTableType");
  }

  @Override public String toString() {
    return "DmTable {" + dmTableName + "}";
  }
  ...
  @Override public <C extends Object> @Nullable C unwrap(Class<C> aClass) {
    if (aClass.isInstance(dmSchema.getDataSource())) {
      return aClass.cast(dmSchema.getDataSource());
    } else if (aClass.isInstance(dmSchema.dialect)) {
      return aClass.cast(dmSchema.dialect);
    } else {
  ...
  private RelProtoDataType supplyProto() {
    try {
      return dmSchema.getRelDataType(
          dmCatalogName,
              dmSchemaName,
              dmTableName);
    } catch (SQLException e) {
      throw new RuntimeException(
          "Exception while reading definition of table '" +  dmTableName
              + "'", e);
    }
  }
  ...
  public SqlIdentifier tableName() {
    ...
    if ( dmSchema.catalog != null) {
      names.add( dmSchema.catalog);
    }
    if ( dmSchema.schema != null) {
      names.add( dmSchema.schema);
    }
    names.add( dmTableName);
    return new SqlIdentifier(names, SqlParserPos.ZERO);
  }

  @Override public RelNode toRel(RelOptTable.ToRelContext context,
      RelOptTable relOptTable) {
    return new DmTableScan(context.getCluster(), context.getTableHints(), relOptTable, this,
            dmSchema.convention);
  }

  @Override public <T> Queryable<T> asQueryable(QueryProvider queryProvider,
      SchemaPlus schema, String tableName) {
    return new DmTableQueryable<>(queryProvider, schema, tableName);
  }

  @Override public Enumerable<@Nullable Object[]> scan(DataContext root) {
    JavaTypeFactory typeFactory = root.getTypeFactory();
    final SqlString sql = generateSql();
    return ResultSetEnumerable.of( dmSchema.getDataSource(), sql.getSql(),
        DmUtils.rowBuilderFactory2(fieldClasses(typeFactory)));
  }
  ...
  @Override public TableModify toModificationRel(RelOptCluster cluster,
    ...
    dmSchema.convention.register(cluster.getPlanner());
  ...
  /** Enumerable that returns the contents of a {@link DmTable} by connecting
   * to the DM data source.
   *
   * @param <T> element type */
  private class DmTableQueryable<T> extends AbstractTableQueryable<T> {
    DmTableQueryable(QueryProvider queryProvider, SchemaPlus schema,
        String tableName) {
      super(queryProvider, schema, DmTable.this, tableName);
    }

    @Override public String toString() {
      return "DmTableQueryable {table: " + tableName + "}";
    }
    ...
    @Override public Enumerator<T> enumerator() {
      ...
      final Enumerable<T> enumerable =
          (Enumerable) ResultSetEnumerable.of( dmSchema.getDataSource(),
              sql.getSql(), DmUtils.rowBuilderFactory2(pairs));
