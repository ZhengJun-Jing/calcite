 ...
 */
public class DmToEnumerableConverter
    extends ConverterImpl
    implements EnumerableRel {
  protected DmToEnumerableConverter(
  ...
  @Override public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
    return new DmToEnumerableConverter(
  ...
  @Override public Result implement(EnumerableRelImplementor implementor, Prefer pref) {
    ...
    final DmRel child = (DmRel) getInput();
    final PhysType physType =
        PhysTypeImpl.of(
            implementor.getTypeFactory(), getRowType(),
            pref.prefer(JavaRowFormat.CUSTOM));
    final DmConvention dmConvention =
        (DmConvention) requireNonNull(child.getConvention(),
            () -> "child.getConvention() is null for " + child);
    SqlString sqlString = generateSql(dmConvention.dialect);
    ...
    final SqlDialect.CalendarPolicy calendarPolicy =
        dmConvention.dialect.getCalendarPolicy();
      ...
      enumerable =
          builder0.append("enumerable",
              Expressions.call(
                  ...
                  Schemas.unwrap(dmConvention.expression, DataSource.class),
                  sql_,
                  rowBuilderFactory_,
    ...
    case ARRAY:
      final Expression x =
          Expressions.convert_(
              Expressions.call(resultSet_, dmGetMethod(primitive),
  ...
  /** E,g, {@code dmGetMethod(int)} returns "getInt". */
  private static String dmGetMethod(@Nullable Primitive primitive) {
  ...
  private SqlString generateSql(SqlDialect dialect) {
    final DmImplementor dmImplementor =
        new DmImplementor(dialect,
            (JavaTypeFactory) getCluster().getTypeFactory());
    final DmImplementor.Result result =
        dmImplementor.visitRoot(this.getInput());
